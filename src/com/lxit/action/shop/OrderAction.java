...
import com.lxit.entity.DeliveryType.DeliveryMethod;
...
import com.lxit.util.ArithUtil;
...
@InterceptorRefs({ @InterceptorRef(value = "token", params = { "excludeMethods", "info,list,view" }),
...
public class OrderAction extends BaseShopAction {
    ...
    public String info() {
        ...
        for (CartItem cartItem : cartItemSet) {
            ...
            productTotalPrice = cartItem.getProduct().getPreferentialPrice(getLoginMember())
                    .multiply(new BigDecimal(cartItem.getQuantity().toString())).add(productTotalPrice);
            Product product = cartItem.getProduct();
        ...
        if (getSystemConfig().getPointType() == PointType.orderAmount) {
            totalPoint = productTotalPrice.multiply(new BigDecimal(getSystemConfig().getPointScale().toString()))
    ...
    // 订单保存
    @Validations(requiredStrings = { @RequiredStringValidator(fieldName = "deliveryType.id", message = "请选择配送方式！") })
    @InputConfig(resultName = "error")
    public String save() {
        cartItemSet = getLoginMember().getCartItemSet();
        if (cartItemSet == null || cartItemSet.size() == 0) {
        ...
        if (deliveryType.getDeliveryMethod() == DeliveryMethod.deliveryAgainstPayment
                && (paymentConfig == null && StringUtils.isEmpty(paymentConfig.getId()))) {
