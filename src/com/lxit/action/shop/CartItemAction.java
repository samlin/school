...
import com.lxit.bean.CartItemCookie;
...
public class CartItemAction extends BaseShopAction {
    ...
    public String ajaxAdd() {
        ...
        if (loginMember == null) {
                ...
                for (Cookie cookie : cookies) {
                    if (StringUtils.equalsIgnoreCase(cookie.getName(), CartItemCookie.CART_ITEM_LIST_COOKIE_NAME)) {
                        if (StringUtils.isNotEmpty(cookie.getValue())) {
                            ...
                            List<CartItemCookie> previousCartItemCookieList = (List<CartItemCookie>) JSONSerializer
                            ...
                            for (CartItemCookie previousCartItemCookie : previousCartItemCookieList) {
                                ...
                                if (StringUtils.equals(previousCartItemCookie.getI(), id)) {
                                    ...
                                    if (product.getStore() != null
                                            && (product.getFreezeStore() + previousCartItemCookie.getQ()) > product
                                                    .getStore()) {
                                ...
                                totalPrice = cartItemCookieProduct.getPreferentialPrice(getLoginMember())
            ...
            if (!isExist) {
                ...
                totalPrice = product.getPreferentialPrice(getLoginMember())
                        .multiply(new BigDecimal(quantity.toString())).add(totalPrice);
                if (product.getStore() != null
            ...
            for (CartItemCookie cartItemCookie : cartItemCookieList) {
                if (StringUtils.equals(cartItemCookie.getI(), id)) {
                    ...
                    if (product.getStore() != null
                            && (cartItemCookieProduct.getFreezeStore() + cartItemCookie.getQ()) > cartItemCookieProduct
                                    .getStore()) {
                        return ajaxJsonErrorMessage("添加购物车失败，商品库存不足!");
                    }
                }
            ...
            if (previousCartItemList != null) {
                for (CartItem previousCartItem : previousCartItemList) {
                    if (StringUtils.equals(previousCartItem.getProduct().getId(), id)) {
                        ...
                        if (product.getStore() != null
                                && (product.getFreezeStore() + previousCartItem.getQuantity()) > product.getStore()) {
                            return ajaxJsonErrorMessage("添加购物车失败，商品库存不足!");
                        }
                        cartItemService.update(previousCartItem);
                    }
                    totalQuantity += previousCartItem.getQuantity();
                    totalPrice = previousCartItem.getProduct().getPreferentialPrice(getLoginMember())
                            .multiply(new BigDecimal(previousCartItem.getQuantity().toString())).add(totalPrice);
                }
            }
            if (!isExist) {
        ...
        if (loginMember == null) {
                ...
                for (Cookie cookie : cookies) {
                    if (StringUtils.equalsIgnoreCase(cookie.getName(), CartItemCookie.CART_ITEM_LIST_COOKIE_NAME)) {
                        if (StringUtils.isNotEmpty(cookie.getValue())) {
                            ...
                            jsonConfig.setRootClass(CartItemCookie.class);
                            JSONArray jsonArray = JSONArray.fromObject(cookie.getValue());
                            List<CartItemCookie> cartItemCookieList = (List<CartItemCookie>) JSONSerializer.toJava(
                            ...
                            for (CartItemCookie cartItemCookie : cartItemCookieList) {
                                ...
                                if (product != null) {
                                    ...
                                    totalPrice = product.getPreferentialPrice(getLoginMember())
                                            .multiply(new BigDecimal(cartItemCookie.getQ().toString())).add(totalPrice);
                                    DecimalFormat decimalFormat = new DecimalFormat(getPriceCurrencyFormat());
                                    String priceString = decimalFormat.format(product
            ...
            if (cartItemSet != null) {
                for (CartItem cartItem : cartItemSet) {
                    ...
                    totalPrice = product.getPreferentialPrice(getLoginMember())
                            .multiply(new BigDecimal(cartItem.getQuantity().toString())).add(totalPrice);
                    DecimalFormat decimalFormat = new DecimalFormat(getPriceCurrencyFormat());
                    String priceString = decimalFormat.format(cartItem.getProduct().getPreferentialPrice(
                            getLoginMember()));
                    Map<String, String> jsonMap = new HashMap<String, String>();
                    jsonMap.put("name", product.getName());
                    jsonMap.put("price", priceString);
    ...
    public String list() {
        ...
        if (loginMember == null) {
            ...
            if (cartItemSet != null) {
                ...
                for (CartItem cartItem : cartItemSet) {
                    ...
                    totalPrice = cartItem.getProduct().getPreferentialPrice(getLoginMember())
                            .multiply(new BigDecimal(cartItem.getQuantity().toString())).add(totalPrice);
                }
            }
        }
        ...
        if (getSystemConfig().getPointType() == PointType.orderAmount) {
            totalPoint = totalPrice.multiply(new BigDecimal(getSystemConfig().getPointScale().toString()))
                        ...
                        if (StringUtils.isNotEmpty(cookie.getValue())) {
                            ...
                            while (iterator.hasNext()) {
                                ...
                                if (StringUtils.equals(id, cartItemCookie.getI())) {
                                    cartItemCookie.setQ(quantity);
                                    subtotalPrice = product.getPreferentialPrice(getLoginMember()).multiply(
                            ...
                            JSONArray jsonArray = JSONArray.fromObject(cartItemCookieList);
                            Cookie newCookie = new Cookie(CartItemCookie.CART_ITEM_LIST_COOKIE_NAME,
                                    jsonArray.toString());
