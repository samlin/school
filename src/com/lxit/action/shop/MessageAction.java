...
public class MessageAction extends BaseShopAction {
    ...
    // 保存消息
    @Validations(requiredStrings = { @RequiredStringValidator(fieldName = "message.title", message = "标题不允许为空!"),
            @RequiredStringValidator(fieldName = "message.content", message = "消息内容不允许为空!") }, stringLengthFields = { @StringLengthFieldValidator(fieldName = "message.content", maxLength = "10000", message = "消息内容长度超出限制！") }, requiredFields = { @RequiredFieldValidator(fieldName = "message.isSaveDraftbox", message = "是否保存草稿箱不允许为空!") })
