...
import com.lxit.bean.SystemConfig;
import com.lxit.bean.SystemConfig.CurrencyType;
import com.lxit.bean.SystemConfig.PointType;
import com.lxit.bean.SystemConfig.RoundType;
import com.lxit.bean.SystemConfig.StoreFreezeTime;
import com.lxit.bean.SystemConfig.WatermarkPosition;
...
public class SystemConfigAction extends BaseAdminAction {
    ...
    @Validations(requiredStrings = {
            ...
            @RequiredStringValidator(fieldName = "systemConfig.shopUrl", message = "网店网址不允许为空!") }, requiredFields = {
            ...
            @RequiredFieldValidator(fieldName = "systemConfig.pointScale", message = "积分换算比率不允许为空!") }, intRangeFields = {
            ...
            @IntRangeFieldValidator(fieldName = "systemConfig.watermarkAlpha", min = "0", max = "100", message = "水印透明度取值范围在${min}-${max}之间!") }, urls = { @UrlValidator(fieldName = "systemConfig.url", message = "网店网址不允许为空!") }, emails = {
    ...
    public String update() throws Exception {
        ...
        if (shopLogo != null || defaultBigProductImage != null || defaultSmallProductImage != null
                || defaultThumbnailProductImage != null || watermarkImage != null) {
            ...
            if (defaultBigProductImage != null) {
                String defaultBigProductImageExtension = StringUtils.substringAfterLast(defaultBigProductImageFileName,
                        ".").toLowerCase();
            ...
            if (defaultSmallProductImage != null) {
                String defaultSmallProductImageExtension = StringUtils.substringAfterLast(
                        defaultSmallProductImageFileName, ".").toLowerCase();
            ...
            if (defaultThumbnailProductImage != null) {
                String defaultThumbnailProductImageExtension = StringUtils.substringAfterLast(
                        defaultThumbnailProductImageFileName, ".").toLowerCase();
            ...
            if (watermarkImage != null) {
                String watermarkImageExtension = StringUtils.substringAfterLast(watermarkImageFileName, ".")
                        .toLowerCase();
        ...
        if (shopLogo != null) {
            File oldShopLogoFile = new File(ServletActionContext.getServletContext().getRealPath(
                    persistent.getShopLogo()));
            if (oldShopLogoFile.isFile()) {
                oldShopLogoFile.delete();
            }
            String shopLogoFilePath = SystemConfig.UPLOAD_IMAGE_DIR + SystemConfig.LOGO_UPLOAD_NAME + "."
                    + StringUtils.substringAfterLast(shopLogoFileName, ".").toLowerCase();
        ...
        // 处理默认商品图片（大）
        if (defaultBigProductImage != null) {
            File oldDefaultBigProductImageFile = new File(ServletActionContext.getServletContext().getRealPath(
                    persistent.getDefaultBigProductImagePath()));
            if (oldDefaultBigProductImageFile.exists()) {
                oldDefaultBigProductImageFile.delete();
            }
            String defaultBigProductImagePath = SystemConfig.UPLOAD_IMAGE_DIR
                    + SystemConfig.DEFAULT_BIG_PRODUCT_IMAGE_FILE_NAME + "."
                    + StringUtils.substringAfterLast(defaultBigProductImageFileName, ".").toLowerCase();
            File defaultBigProductImageFile = new File(ServletActionContext.getServletContext().getRealPath(
                    defaultBigProductImagePath));
        ...
        // 处理默认商品图片（小）
        if (defaultSmallProductImage != null) {
            File oldDefaultSmallProductImageFile = new File(ServletActionContext.getServletContext().getRealPath(
                    persistent.getDefaultSmallProductImagePath()));
            if (oldDefaultSmallProductImageFile.exists()) {
                oldDefaultSmallProductImageFile.delete();
            }
            String defaultSmallProductImagePath = SystemConfig.UPLOAD_IMAGE_DIR
                    + SystemConfig.DEFAULT_SMALL_PRODUCT_IMAGE_FILE_NAME + "."
                    + StringUtils.substringAfterLast(defaultSmallProductImageFileName, ".").toLowerCase();
            File defaultSmallProductImageFile = new File(ServletActionContext.getServletContext().getRealPath(
        ...
        // 处理默认商品缩略图
        if (defaultThumbnailProductImage != null) {
            File oldDefaultThumbnailProductImageFile = new File(ServletActionContext.getServletContext().getRealPath(
                    persistent.getDefaultThumbnailProductImagePath()));
            if (oldDefaultThumbnailProductImageFile.exists()) {
                oldDefaultThumbnailProductImageFile.delete();
            }
            String defaultThumbnailProductImagePath = SystemConfig.UPLOAD_IMAGE_DIR
                    + SystemConfig.DEFAULT_THUMBNAIL_PRODUCT_IMAGE_FILE_NAME + "."
                    + StringUtils.substringAfterLast(defaultThumbnailProductImageFileName, ".").toLowerCase();
            File defaultThumbnailProductImageFile = new File(ServletActionContext.getServletContext().getRealPath(
                    defaultThumbnailProductImagePath));
        ...
        // 处理水印图片
        if (watermarkImage != null) {
            File oldWatermarkImageFile = new File(ServletActionContext.getServletContext().getRealPath(
                    persistent.getWatermarkImagePath()));
            if (oldWatermarkImageFile.exists()) {
                oldWatermarkImageFile.delete();
            }
            String watermarkImagePath = SystemConfig.UPLOAD_IMAGE_DIR + SystemConfig.WATERMARK_IMAGE_FILE_NAME + "."
                    + StringUtils.substringAfterLast(watermarkImageFileName, ".").toLowerCase();
        ...
        BeanUtils.copyProperties(systemConfig, persistent, new String[] { "systemName", "systemVersion",
                "systemDescription", "isInstalled", "shopLogo", "defaultBigProductImagePath",
                "defaultSmallProductImagePath", "defaultThumbnailProductImagePath", "watermarkImagePath" });
