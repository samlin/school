...
import com.lxit.service.FriendLinkService;
...
public class FriendLinkAction extends BaseAdminAction {
    ...
    // 保存
    @Validations(requiredStrings = { @RequiredStringValidator(fieldName = "friendLink.name", message = "友情链接名称不允许为空!"),
            @RequiredStringValidator(fieldName = "friendLink.url", message = "链接地址不允许为空!") }, requiredFields = { @RequiredFieldValidator(fieldName = "friendLink.orderList", message = "排序不允许为空!") }, intRangeFields = { @IntRangeFieldValidator(fieldName = "friendLink.orderList", min = "0", message = "排序必须为零或正整数!") })
    ...
    public String save() throws Exception {
        ...
        if (logo != null) {
            String logoFilePath = SystemConfig.UPLOAD_IMAGE_DIR + CommonUtil.getUUID() + "."
                    + StringUtils.substringAfterLast(logoFileName, ".").toLowerCase();
    ...
    public String update() throws Exception {
        ...
        if (logo != null) {
            ...
            if (persistent.getLogo() != null) {
                ...
                logoFilePath = SystemConfig.UPLOAD_IMAGE_DIR + CommonUtil.getUUID() + "."
                        + StringUtils.substringAfterLast(logoFileName, ".").toLowerCase();
            }
            File logoFile = new File(ServletActionContext.getServletContext().getRealPath(logoFilePath));
            FileUtils.copyFile(logo, logoFile);
