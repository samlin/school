...
public class ArticleAction extends BaseAdminAction {
    ...
    // 保存
    @Validations(requiredStrings = { @RequiredStringValidator(fieldName = "article.title", message = "标题不允许为空!"),
            @RequiredStringValidator(fieldName = "article.articleCategory.id", message = "文章分类不允许为空!"),
            @RequiredStringValidator(fieldName = "article.content", message = "文章内容不允许为空!") }, requiredFields = {
    ...
    public String update() throws Exception {
        ...
        BeanUtils.copyProperties(article, persistent, new String[] { "id", "createDate", "modifyDate", "pageCount",
                "htmlFilePath", "hits" });
