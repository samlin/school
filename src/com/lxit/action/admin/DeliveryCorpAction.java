...
public class DeliveryCorpAction extends BaseAdminAction {
    ...
    // 保存
    @Validations(requiredStrings = { @RequiredStringValidator(fieldName = "deliveryCorp.name", message = "物流公司名称不允许为空!") }, requiredFields = { @RequiredFieldValidator(fieldName = "deliveryCorp.orderList", message = "排序不允许为空!") }, intRangeFields = { @IntRangeFieldValidator(fieldName = "deliveryCorp.orderList", min = "0", message = "排序必须为零或正整数!") }, urls = { @UrlValidator(fieldName = "deliveryCorp.url", message = "网址格式错误!") })
    ...
    public String update() {
        ...
        BeanUtils.copyProperties(deliveryCorp, persistent, new String[] { "id", "createDate", "modifyDate",
                "deliveryTypeSet" });
