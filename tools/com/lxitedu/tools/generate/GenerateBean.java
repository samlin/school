...
public class GenerateBean extends BaseGenerate implements Observer {
     ...
     */
    public GenerateBean(GeneralSubject subject) {
        subject.attach(this);
    }
    ...
    public static void printGetSetMethod() throws Exception {
        for (int i = 1; i <= rsmd.getColumnCount(); i++) {
            if (exceptionFields.get(rsmd.getColumnName(i)) == null) {
                ...
                // set method.
                out.println();
                out.println("  public void set" + getVariableName(rsmd.getColumnName(i), true) + "("
                        + getJavaType(rsmd, i) + " " + getVariableName(rsmd.getColumnName(i)) + ") { ");
        ...
        for (int i = 1; i <= rsmd.getColumnCount(); i++) {
            ...
            if (exceptionFields.get(rsmd.getColumnName(i)) == null) {
                if (JAVA_INTEGER.equals(getJavaType(rsmd, i))) { // int
                    out.println("  private " + getJavaType(rsmd, i) + " " + getVariableName(rsmd.getColumnName(i))
                            + " = null;");
